...
/* ── Fake hex/binary strings for the animation ── */
...
export default function BreachGateway({
  children,
}: {
  ...
  // Generate hex blocks only on client to avoid hydration mismatch
  useEffect(() => {
    setMatrixCols(
  ...
  /* ── Start the breach sequence ── */
  const initiateBreach = useCallback(() => {
    if (phase !== "locked") return;
    setPhase("breaching");
    playSfx("glitch");

    // Type out each scan line with progress
    const totalDuration = 2400; // total breach time in ms
    ...
    SCAN_LINES.forEach((line, i) => {
      const t = setTimeout(() => {
        ...
        setHexStream((prev) => [
          ...prev.slice(-6),
          `0x${randHex(4)} → 0x${randHex(4)} [${
            i < SCAN_LINES.length - 1 ? "PROC" : "DONE"
          }]`,
        ]);
      }, i * lineDelay);
  ...
  return (
    <>
      {/* ── Breach Section (only when not unlocked) ── */}
      <AnimatePresence>
        {phase !== "unlocked" && (
          <motion.section
            key="breach"
            ...
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="text-center mb-8">
                ...
                <p className="font-mono text-sm text-slate-500">
                  {">"} Initiate security breach to access classified projects
                </p>
              </div>
              ...
              >
                ...
                <div className="relative bg-[#080810] p-5 min-h-[260px]">
                  {/* Background hex matrix (CSS only, no JS animation) */}
                  <div className="absolute inset-0 overflow-hidden opacity-[0.06] pointer-events-none">
                    {matrixCols.map((col, i) => (
                      <div
                        key={i}
                        className="absolute top-0 font-mono text-[10px] text-neon-green leading-[14px] whitespace-nowrap"
                        style={{
                          left: `${col.left}%`,
                          animation: `matrixDrop ${col.speed}s linear ${col.delay}s infinite`,
                        }}
                      >
                        {col.chars.map((c, j) => (
                          <div key={j}>{c}</div>
                  ...
                  {phase === "breaching" && (
                    <div
                      ...
                      style={{
                        background:
                          "repeating-linear-gradient(0deg, transparent 0px, transparent 2px, rgba(0,240,255,0.015) 2px, rgba(0,240,255,0.015) 4px)",
                      }}
                    />
                  )}
                  ...
                  {phase === "locked" && (
                    <div className="relative z-10 flex flex-col items-center justify-center h-full min-h-[200px] gap-5">
                      ...
                      <div className="text-center">
                        <p className="font-gaming text-sm tracking-widest text-slate-400 mb-1">
                          CLASSIFIED PROJECTS
                        </p>
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.96 }}
                        className="relative gaming-btn liquid-btn !px-10 !py-3.5 group"
                      >
                  ...
                  {phase === "breaching" && (
                    <div className="relative z-10 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="font-gaming text-[10px] tracking-widest text-neon-red animate-flicker">
                          ⚠ BREACH IN PROGRESS
                        </span>
                      ...
                      <div className="relative h-1.5 bg-gaming-dark rounded-full overflow-hidden">
                        <motion.div
                          ...
                          style={{
                            background:
                              "linear-gradient(90deg, #00f0ff, #a855f7, #ff00e5)",
                            boxShadow:
                          ...
                          <div key={i} className="flex items-center gap-3">
                            <div
                              className={`w-2.5 h-2.5 rounded-full border transition-all duration-500 ${
                                i <= scanLine
                                  ? "bg-neon-cyan border-neon-cyan shadow-[0_0_8px_rgba(0,240,255,0.6)]"
                                  : "border-slate-700"
                              }`}
                            />
      ...
      {/* ── Locked placeholder (prevents layout jump before breach) ── */}
      {phase === "locked" && <div className="h-0" />}

      {/* CSS for matrix rain (pure CSS, no JS overhead) */}
      <style jsx>{`

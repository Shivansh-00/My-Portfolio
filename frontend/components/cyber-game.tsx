  ...
  const spawnTarget = useCallback(() => {
    ...
    setTargets((prev) => {
      ...
      const newTarget: Target = {
        ...
        size,
        speed: (type === "fast" ? 2 : 1) * config.speedMul,
  ...
  const handleTargetClick = useCallback(
    (target: Target, e: React.MouseEvent) => {
      ...
      if (target.type === "danger") {
        ...
        setScore((s) => s + target.points + comboBonus);
        setCombo((c) => {
          const newCombo = c + 1;
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-mono text-sm text-slate-500">
            {">"} Eliminate cyber threats // Avoid red targets // Build combos
          </p>
        </div>

        {/* Game Container */}
        <div className="gaming-card !p-0 overflow-hidden holo-shimmer">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gaming-border bg-gaming-dark/60">
            <div className="flex items-center gap-6">
              ...
              <div className="font-gaming text-xs tracking-widest">
                ...
                <span className={combo > 5 ? "neon-text-magenta" : combo > 2 ? "neon-text-green" : "text-slate-300"}>
                  x{combo}
                </span>
              </div>
            ...
            <div className="flex items-center gap-6">
              <div className="font-gaming text-xs tracking-widest">
                ...
                <span className={timeLeft <= 10 ? "text-neon-red animate-flicker" : "text-slate-300"}>
                  {timeLeft}s
                </span>
              </div>
          ...
          >
            ...
            {gameState === "idle" && (
              ...
              >
                ...
                <div className="text-center">
                  <h3 className="font-gaming text-xl uppercase tracking-widest neon-text-cyan mb-2">
                    Neural Defense v2.0
                  </h3>
                  <p className="font-mono text-xs text-slate-500 max-w-xs">
                    Click cyan/magenta/green targets for points.
                    <br />
                    <span className="text-neon-red">Avoid RED targets</span> — they cost lives!
                  </p>
            ...
            <AnimatePresence>
              {targets.map((target) => {
                ...
                const lifeRatio = 1 - elapsed / target.lifetime;
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${target.color}80`,
                        background: `${target.color}30`,
                      }}
                    />

                    {/* Danger icon */}
                    {target.type === "danger" && (
                      <span className="absolute font-gaming text-[10px] text-neon-red" style={{ top: -14 }}>
                        ⚠
                      </span>
                    )}
                    ...
                    {/* Bonus marker */}
                    {target.type === "bonus" && (
                      <span className="absolute font-gaming text-[10px] text-neon-green" style={{ top: -14 }}>
                        ★
                      </span>
                    )}
            ...
            <AnimatePresence>
              {showLevelUp && (
                ...
                >
                  <div className="text-center">
                    <div className="font-gaming text-3xl neon-text-green tracking-widest animate-bounce-in">
                      LEVEL UP!
                    </div>
                    <div className="font-gaming text-lg text-neon-orange tracking-wider mt-2">
                      {config.name}
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-8 text-left">
                      <div>
                        <span className="font-mono text-xs text-slate-500">FINAL SCORE</span>
                        <p className="font-gaming text-lg neon-text-cyan">{score.toLocaleString()}</p>
                      </div>
                      <div>
                        <span className="font-mono text-xs text-slate-500">MAX COMBO</span>
                        <p className="font-gaming text-lg neon-text-magenta">x{maxCombo}</p>
                      </div>
                      <div>
                        <span className="font-mono text-xs text-slate-500">ACCURACY</span>
                        <p className="font-gaming text-lg neon-text-green">{accuracy}%</p>
                      </div>
                      <div>
                        <span className="font-mono text-xs text-slate-500">LEVEL</span>
                        <p className="font-gaming text-lg text-neon-orange">{config.name}</p>
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between px-4 py-2 border-t border-gaming-border bg-gaming-dark/60">
            <div className="font-mono text-[10px] text-slate-600">
              TARGETS: {hits} HIT / {misses} MISS
            </div>
            <div className="font-mono text-[10px] text-slate-600">
              ACC: {accuracy}% | HI: {highScore.toLocaleString()}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-6">
          {[
            { color: "#00f0ff", label: "STANDARD", pts: "+100" },
            { color: "#ff00e5", label: "FAST", pts: "+150" },
            ...
            <div key={item.label} className="flex items-center gap-2">
              ...
              <span className="font-mono text-[10px] text-slate-500">
                {item.label}{" "}
                <span style={{ color: item.color }}>{item.pts}</span>
              </span>
            </div>
          ))}
        </div>
